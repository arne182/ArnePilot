  ...
  def update(self, CS, CP, VM, PP, live20, live100, md, live_map_data):
    ...
    if enabled:
      ...
      if gasbuttonstatus == 0:
        ...
        jerk_limits = [min(-0.05, accel_limits[0] * 0.5), max(accellimitmaxdynamic, accel_limits[1])]  # dynamic
      elif gasbuttonstatus == 1:
        accellimitmaxsport = -0.002*v_ego+0.4
        jerk_limits = [min(-0.05, accel_limits[0]), max(accellimitmaxsport, accel_limits[1])]  # sport
      elif gasbuttonstatus == 2:
        accellimitmaxeco = -0.0015*v_ego+0.1
        jerk_limits = [min(-0.05, accel_limits[0] * 0.5), max(accellimitmaxeco, accel_limits[1])]  # eco
